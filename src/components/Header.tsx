    ...
    <header className="sticky top-0 z-40 backdrop-blur-xl bg-[var(--bg)]/85 border-b border-soft">
      <div className="mx-auto max-w-7xl px-4 py-3 flex items-center gap-3">
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          <div className="hidden sm:block"><h1 className="section-title text-lg leading-none text-accent">AdhyatmaVeda</h1><p className="text-[10px] text-muted leading-tight mt-0.5">Sadhana Junction</p></div>
        </div>
        <div className="flex-1 max-w-xl mx-auto">
          <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted" /><input type="text" value={searchQuery} onChange={(e) => onSearch(e.target.value)} placeholder="Search mantras, texts, deities, folk lore…" className="input-field pl-10 text-sm" /></div>
        </div>
